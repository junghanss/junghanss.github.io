...
fecha_hoy <- "29/08/2023"
# Con Parsers de readr package:
parse_date(fecha_hoy, format = "%m/%d/%Y")  # formato month day year (con 4 digitos)
parse_date("29/08/23", format = "%d/%m/%y")  # formato day month year
parse_date("29/08/23", format = "%y/%m/%d")  # formato year month day
parse_date("29 Agosto 2023", "%d %B %Y", locale = locale("es"))  # formato day month year con el mes en palabra, por eso seteamos lenguaje ESPAÑOL
...
dataset_individual_2023 <- get_microdata(year = 2023, trimester = 1, type = 'individual')
glimpse(dataset_individual_2023$CH05) # Vemos en qué formato viene
class(dataset_individual_2023$CH05)  # El type nativo de la variable es char
dataset_individual_2023 <- dataset_individual_2023 %>% rename(FEC_NAC = CH05) # Renombramos la variable de fecha de nacimiento
  
dataset_dates <- dataset_individual_2023 %>% 
